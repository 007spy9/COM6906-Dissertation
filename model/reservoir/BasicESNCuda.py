...
class BasicESNCuda:
    def __init__(self, leakage_rate, spectral_radius, gamma, n_neurons, W_in, is_optimising=False):
        ...
        self.spectral_radius = tf.Variable(spectral_radius, trainable=False, dtype=tf.float32)
        self.gamma = tf.Variable(gamma, trainable=False, dtype=tf.float32)
        self.N = n_neurons
        self.W_in = tf.convert_to_tensor(W_in, dtype=tf.float32)

        self.is_optimising = is_optimising

        self.W_res = tf.convert_to_tensor(np.random.uniform(-1, 1, (self.N, self.N)), dtype=tf.float32)
